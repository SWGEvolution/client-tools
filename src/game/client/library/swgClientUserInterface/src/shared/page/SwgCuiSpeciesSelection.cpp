...
namespace SwgCuiSpeciesSelectionNamespace
{
	...
	std::string const serverTemplatePrefix = "object/creature/player/";
	std::string const serverTemplateSuffix = ".iff";
	...
	void deleteAvatarMap(SwgCuiSpeciesSelection::AvatarMap * & avatarMap);
	...
	std::string getServerName(std::string const & templateName);
...
void SwgCuiSpeciesSelectionNamespace::verifyMessageBoxClosed(const CuiMessageBox & box)
{
...
int SwgCuiSpeciesSelectionNamespace::cutString(Unicode::String const & str, Unicode::String & one, Unicode::String & two)
{
...
CreatureObject * SwgCuiSpeciesSelectionNamespace::createPlayerModel(std::string const & templateName)
{
...
void SwgCuiSpeciesSelectionNamespace::deleteAvatarMap(SwgCuiSpeciesSelection::AvatarMap * & avatarMap)
{
	{
		CreatureObject const * const player = SwgCuiAvatarCreationHelper::getCreature();

		SwgCuiSpeciesSelection::AvatarMap::iterator ii = avatarMap->begin();
		SwgCuiSpeciesSelection::AvatarMap::iterator iiEnd = avatarMap->end();

		for (; ii != iiEnd; ++ii)
		{
			SwgCuiSpeciesSelection::Avatar & avatar = ii->second;
			...
			{
				SwgCuiSpeciesSelection::TangibleMap::iterator jj = avatar.m_hairStyles.begin();
				SwgCuiSpeciesSelection::TangibleMap::iterator jjEnd = avatar.m_hairStyles.end();
...
TangibleObject * SwgCuiSpeciesSelectionNamespace::createHair(std::string const & templateName, CustomizationData * m_cdata)
{
...
void SwgCuiSpeciesSelectionNamespace::randomizeHairColors(TangibleObject * const hair, std::string const & varName, int const value)
{
	if (varName.empty())
	{
		return;
...
void SwgCuiSpeciesSelectionNamespace::abortBackToSelection()
{
...
void SwgCuiSpeciesSelectionNamespace::onMessageBoxClosedConfirmCancel(const CuiMessageBox & box)
{
...
std::string SwgCuiSpeciesSelectionNamespace::getCodeDataName(std::string const & templateName)
{
...
std::string SwgCuiSpeciesSelectionNamespace::getServerName(std::string const & templateName)
{
	std::string::size_type begin = templateName.rfind("shared_") + strlen("shared") + 1;
	std::string::size_type end = templateName.rfind(".iff");
	...
	codeDataName = templateName.substr(begin, end - begin);
	codeDataName = (serverTemplatePrefix + codeDataName + serverTemplateSuffix);
...
using namespace SwgCuiSpeciesSelectionNamespace;

//======================================================================

struct SwgCuiSpeciesSelection::SpeciesButton
{
	int m_index;
...
SwgCuiSpeciesSelection::SpeciesButton::SpeciesButton()
: m_index(0)
, m_maleCodeDataName()
...
SwgCuiSpeciesSelection::SwgCuiSpeciesSelection(UIPage & page)
: CuiMediator("SwgCuiSpeciesSelection", page)
, UIEventCallback()
, m_buttonBack(0)
, m_buttonNext(0)
...
SwgCuiSpeciesSelection::~SwgCuiSpeciesSelection()
{
...
void SwgCuiSpeciesSelection::performActivate()
{
	...
	m_callback->connect(*this, &SwgCuiSpeciesSelection::onCreationAborted, static_cast<SwgCuiAvatarCreationHelper::Messages::Aborted*>(0));
	m_callback->connect(*this, &SwgCuiSpeciesSelection::onCreationFinished, static_cast<SwgCuiAvatarCreationHelper::Messages::CreationFinished *>(0));
...
void SwgCuiSpeciesSelection::performDeactivate()
{
	...
	m_callback->disconnect(*this, &SwgCuiSpeciesSelection::onCreationAborted, static_cast<SwgCuiAvatarCreationHelper::Messages::Aborted*>(0));
	m_callback->disconnect(*this, &SwgCuiSpeciesSelection::onCreationFinished, static_cast<SwgCuiAvatarCreationHelper::Messages::CreationFinished *>(0));
...
void SwgCuiSpeciesSelection::OnButtonPressed(UIWidget * const context)
{
	...
	{
		CuiMessageBox::createInfoBox (Unicode::narrowToWide ("You will now be disconnected from the server. Please log out and log back in."));
		setSpeciesTemplate();
	}
...
void SwgCuiSpeciesSelection::onCreationAborted(bool)
{
	m_aborted = true;
...
void SwgCuiSpeciesSelection::onCreationFinished(bool b)
{
	if (b)
	{
		if (m_aborted)
...
void SwgCuiSpeciesSelection::update(float deltaTimeSecs)
{
...
void SwgCuiSpeciesSelection::prefetchAnotherObject()
{
	{
...
void SwgCuiSpeciesSelection::updateAvatarSelection()
{
...
void SwgCuiSpeciesSelection::loadAllPlayerModelData()
{
...
void SwgCuiSpeciesSelection::initializeCustomization()
{
	CreatureObject * const playerAvatar = SwgCuiAvatarCreationHelper::getCreature();
	NOT_NULL(playerAvatar);
...
void SwgCuiSpeciesSelection::randomize()
{
...
void SwgCuiSpeciesSelection::randomizeGender()
{
...
void SwgCuiSpeciesSelection::selectGender(Gender gender)
{
	m_gender = gender;
	updateAvatarSelection();
...
void SwgCuiSpeciesSelection::selectSpecies(Direction const direction)
{
	int const rowCount = m_hiddenAvatarList->GetRowCount();
...
void SwgCuiSpeciesSelection::selectSpecies(SpeciesButton const * speciesButton)
{
	NOT_NULL(speciesButton);
...
void SwgCuiSpeciesSelection::selectSpecies(int const newSelection)
{
	int const rowCount = m_hiddenAvatarList->GetRowCount();
...
void SwgCuiSpeciesSelection::randomizeAppearance()
{
...
void SwgCuiSpeciesSelection::updateSpeciesDescription()
{
	UIData const * const selectedData = m_hiddenAvatarList->GetDataAtRow(m_hiddenAvatarList->GetLastSelectedRow());
...
void SwgCuiSpeciesSelection::setSpeciesTemplate ()
{
	UIData const * const selectedData = m_hiddenAvatarList->GetDataAtRow(m_hiddenAvatarList->GetLastSelectedRow());

	//Unicode::wideToNarrow(templateName);	

	NOT_NULL(selectedData);
	{
		UINarrowString templateName;
		selectedData->GetPropertyNarrow(m_gender == G_Male ? s_maleProp : s_femaleProp, templateName);
		std::string serverName = getServerName(templateName);
		...
		if (controller)
		{
			MessageQueueChangeSpecies * const msgSetSpeciesTemplate = new MessageQueueChangeSpecies(serverName);
			controller->appendMessage(	CM_setSpeciesTemplate, 
											0.0f, 
											msgSetSpeciesTemplate, 
...
void SwgCuiSpeciesSelection::finishedSendingMessage ()
{
	//GameNetwork::disconnectConnectionServer();
	closeNextFrame();
	unregisterMediatorObjects();
